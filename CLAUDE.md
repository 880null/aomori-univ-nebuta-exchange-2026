...
- 実装だけでなく、**動作検証・調査**も可能な限り `/codex:rescue` に含めて一括委任すること。
  Claude自身がBashで直接検証コマンドを実行するのは、Codexへの委任が明らかに
  非効率な場合（1コマンドで完結する軽い確認等）に限定する。
  委任時は「何をどう検証し、どの数値を報告するか」まで指示に含めること。
  - **Codexに委任できる検証**（ファイル読み書きだけで完結するもの）:
    型チェック（`npx tsc --noEmit`）、データ分析（CSV/JSONの集計・突合）、
    生成物の静的検査（`out/` のHTMLの中身の確認）、ソースの整合性チェック。
  - **Codexに委任できない検証**: ブラウザでの実地テスト、レイアウト計測、
    スクリーンショット、HTTPアクセスを伴う確認。
    **Codexのサンドボックスはlocalhostを含む全ネットワークを遮断している**
    （`localhost:8899` / `localhost:9333` いずれも curl 終了コード7で到達不可。
    同時刻にClaude側からは両方200で応答することを確認済み）。
    これらはClaudeがChrome DevTools Protocol経由で実施する。
